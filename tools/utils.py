...
def estimate_pose(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    ...
    # f_mean = np.max([K0[0, 0], K1[1, 1], K0[0, 0], K1[1, 1]])
...
def estimate_pose_m(kpts0, kpts1, K0, K1, norm_thresh, conf=0.99999, method=cv2.RANSAC):
    if len(kpts0) < 5:
        return None
...
def estimate_pose_m_v2(kpts0, kpts1, K0, K1, norm_thresh, conf=0.99999, method=cv2.RANSAC, mask=None):
    if len(kpts0) < 5:
        return None
...
# def estimate_pose_magsac(kpts0, kpts1, K0, K1, h0, w0, h1, w1, thresh=1.0, probabilities=None):
#     if len(kpts0) < 5:
#         return None
#     kpts1 = (kpts1 - K1[[0, 1], [2, 2]][None]) / K1[[0, 1], [0, 1]][None]
#     norm_kpts0 = (kpts0 - K0[[0, 1], [2, 2]][None]) / K0[[0, 1], [0, 1]][None]
#     norm_kpts1 = (kpts1 - K1[[0, 1], [2, 2]][None]) / K1[[0, 1], [0, 1]][None]
#
#     # correspondences = np.hstack([norm_kpts0, norm_kpts1])
#     correspondences = np.hstack([kpts0, kpts1])
#
#     F, mask = pymagsac.findFundamentalMatrix(
#         np.ascontiguousarray(correspondences).astype(np.float64),
...
#         float(w1),
#         float(h1),
#         probabilities=np.array([1.0 for i in range(kpts0.shape[0])],
#                                dtype=np.float64),
#         # sampler=sampler_id,
...
#         return None
#
#     print('F_magsac: ', F, np.sum(mask))
#
#     # cv2.triangulatePoints()
#
#     E = K1.T @ F @ K0
#     n, R, t, mask_new = cv2.recoverPose(
#         E, kpts0, kpts1, np.eye(3), 1e9, mask=np.array(mask, dtype=np.uint8).reshape(-1, 1))
#     ret = (R, t[:, 0], mask.ravel() > 0)
#     return ret

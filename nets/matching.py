...
class Matching(torch.nn.Module):
    ...
    def __init__(self, config={}, superpoint=None, superglue=None):
        super().__init__()
        if superpoint is None:
            self.superpoint = SuperPoint(config.get('superpoint', {})).eval()
            print('Initialized superpoint from config')
        else:
            self.superpoint = superpoint.eval()

        if superglue is None:
            self.superglue = SuperGlue(config.get('superglue', {}))
            print('Initialized superglue from config')
        else:
